...
class MapReduce(object):
    ...
    def run(self):
        ...
        regions = pool.map(self.doMap, list(range(0, self.maptask)))
        partitions = pool.map(self.doReduce, list(range(0, self.reducetask)))
...
class SyntacticVariants(MapReduce):
    '''
    find words in similar contexts

    '''

    def __init__(self, maptask, reducetask, path, context_width):
    ...
    def Map(self, keyvalue, value):
        results = []
        L = value.split(' ')
        for x in range(self.context_width):
            L = ["<START>"] + L
            L.append('<END>')

        for i, w in enumerate(L[self.context_width : len(L) - self.context_width]):
            position = i + self.context_width
            # True if w contains non-alphanumeric characters
            l_context = L[position-self.context_width : position]

            r_context = L[position+1:position+1+self.context_width]

            results.append((tuple(l_context + r_context), w))
    ...
    def Reduce(self, key, keyvalues):
        # return (key, sum(pair[1] for pair in keyvalues))
        return (key, dict((x, [p[0] for p in keyvalues].count(x)) for x in set(p[0] for p in keyvalues)))
...
if __name__ == '__main__':
    ...
    # Create a SyntacticVariant MapReduce program
    wc = SyntacticVariants(4, 2, sys.argv[1], 2)
    # Run it
    wc.run()
